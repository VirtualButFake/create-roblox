...
async function getPattern(
    ...
    switch (os.platform()) {
        case 'win32':
            return (
                patternOverwrites?.win32 ||
                `^${folderName}(?:-|-.*-)windows-x86_64$`
            );
        case 'darwin':
            return (
                patternOverwrites?.darwin ||
                `^${folderName}(?:-|-.*-)macos-x86_64$`
            );
        case 'linux':
            return (
                patternOverwrites?.linux ||
                `^${folderName}(?:-|-.*-)linux-x86_64$`
            );
        default:
            return undefined;
    }
